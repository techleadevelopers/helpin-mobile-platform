...
const BookingDetailsModal = ({ isOpen, onClose, bookingId }: BookingDetailsModalProps) => {
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Sucesso!", description: "Status do caso atualizado." });
      setSelectedStatus("");
      setNotes("");
    },
    onError: (error: any) => {
  ...
  const handleForceConfirmPayment = () => {
    ...
    if (!window.confirm("Confirma manualmente o pagamento PIX para este caso?")) {
      return;
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Detalhes do Caso: {booking?.id.substring(0, 8)}...</DialogTitle>
        </DialogHeader>
        {isLoading ? (
          <div className="text-center py-8">Carregando detalhes do caso...</div>
        ) : isError ? (
          <div className="text-center py-8 text-red-600">Erro ao carregar caso: {error?.message}</div>
        ) : booking ? (
            ...
            <CardContent className="pt-6">
              {isInitialLoading ? (
                ...
                <div className="text-center py-12 text-red-600">
                  <p>Erro ao carregar casos: {error?.message}</p>
                </div>
              ) : bookings.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500">
                    {debouncedSearchTerm
                      ? `Nenhum caso corresponde a "${debouncedSearchTerm}"`
                              ...
                              <div className="flex items-center space-x-4">
                                ...
                                <div className="flex-1">
                                  <div className="flex items-center gap-3 mb-1">
                                    <h3 className="font-semibold text-gray-900">Caso #{booking.id.substring(0, 8)}</h3>
